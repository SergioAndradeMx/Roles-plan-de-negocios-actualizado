...
<head>
    ...
    @vite(['resources/css/estilos.css', 'resources/css/app.css', 'resources/js/app.js', 'resources/js/costoFijo.js', 'resources/js/menuIzquierdo'])
</head>
...
<body class="">
    ...
    <div class="flex justify-center items-start 2xl:px-10 flex-wrap gap-10 w-full h-full ">
        ...
        <div class="card w-3/4  mt-3">
            <h2 class="text-center text-2xl dark:text-white my-5">Progreso de costos fijos</h2>
            <hr class="my-2 h-0.5 border-t-0 w-full bg-neutral-100 dark:bg-white m-0 p-0" />
            <div class="px-2 pb-2 mx-2 mb-2 bg-white">
                <h2 class="text-center py-4  2xl:text-2xl text-lg font-normal">Ingresa el costo fijo</h2>
                <table class="w-full table-auto" id="miTabla" dato='{{ $plan_de_negocio->id }}'>
                        ...
                        @foreach ($costos_fijos as $costo_fijo)
                            <tr>
                                <td class="border px-4 py-2"> <input type="text"
                                        class="w-full border rounded-sm px-2 py-1" value="{{ $costo_fijo->nombre }}">
                                </td>
                                <td class="border px-4 py-2"><input type="text"
                                ...
                                <td class="border px-4 py-2"><input type="text"
                                        class="w-full border rounded-sm px-2 text-right py-1"
                                        value="{{ sprintf("%.2f", $costo_fijo->valor_unitario * $costo_fijo->monto_unitario)}}" disabled></td>
                                <td> <button
                    ...
                    <tfoot class="">
                        <tr>
                            <td id="costosFijos" colspan="5" class="text-right pr-2 dark:bg-gray-800  text-white">
