...
<head>
    ...
    @vite(['resources/css/app.css', 'resources/js/app.js', 'resources/js/proyeccionOptimista.js'])
    <title>Proyección anual optimista</title>
</head>
...
<body>
    ...
    <div class="flex items-center flex-col text-black my-2 sm:my-4">
        ...
        <h2>Proyección anual optimista</h2>
    </div>

    <div class="container mx-auto card">
        <h2 class="text-center text-2xl dark:text-white my-5">Proyección anual optimista</h2>
        <hr class="my-2 h-0.5 border-t-0 w-full bg-neutral-100 dark:bg-white m-0 p-0" />
        <div class="px-2 pb-2 mx-2 mb-2 bg-white">
            <h3 class="text-center py-2 2xl:text-2xl text-lg font-normal">Proyección anual optimista</h3>
            <table class="w-full table-auto" id="miTabla" dato='{{ $plan_de_negocio->id }}'>
                    ...
                    @else
                        @foreach ($costos_fijos as $costo_fijo)
                            <tr>
                                ...
                                @for ($i = 1; $i <= 12; $i++)
                                    <td class="border "><input type="text" class="w-full border rounded-sm text-xs px-0 text-right"
                ...
                <thead>
                    ...
                    <tr id="costosFjo">
                        <th class="border text-center text-xs border-gray-500 dark:bg-gray-400  text-black" width="7.5%">
                            Total de costos fijos: </th>
                        <th class="border text-right text-xs border-gray-500 dark:bg-gray-400 text-black"
                            width="7.5%"></th>
                        <th class="border text-right text-xs border-gray-500 dark:bg-gray-400 text-black"
                            width="7.5%"></th>
                        <th class="border text-right text-xs border-gray-500 dark:bg-gray-400 text-black"
                ...
                <tbody id="tdbodyVariables">
                    @if (count($costosVariablesAgrupados) > 0)
                        ...
                        @foreach ($costosVariablesAgrupados as $id => $costoVariable)
                            @foreach ($costoVariable as $name => $data)
                                <tr>
                            ...
                            <tr>
                                ...
                                @for ($i = 1; $i <= 12; $i++)
                                    <td class="border "><input type="text"
                                            ...
                                            value="{{ $costos_variable->escenario_optimista }}">
                                    </td>
                                @endfor
                            </tr>
                        @endforeach
                ...
                <thead>
                    ...
                    <thead>
                        <tr>
                            <th class="border text-right pr-2 border-gray-500 dark:bg-gray-800  text-white"
                                width="15%" colspan="2">
                                Mes 1</th>
                            <th class="border border-gray-500 dark:bg-gray-800  text-white" width="7.5%">Mes 2</th>
                            <th class="border border-gray-500 dark:bg-gray-800  text-white" width="7.5%">Mes 3</th>
                    ...
                    @else
                        ...
                        @foreach ($ingresos as $ingreso)
                            <tr>
                                ...
                                @for ($i = 1; $i <= 12; $i++)
                                    <td class="border "><input type="text"
                                            class="w-full border rounded-sm px-0 text-xs text-right"
                                            value="{{ $ingreso->escenario_optimista }}">
                                    </td>
                                @endfor
                            </tr>
                        @endforeach
